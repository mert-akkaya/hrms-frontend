...
export default function JobAdvertisementConfirm() {
  ...
  const deleteJobAdvertisement = (jobAdvertisement)=>{
    let jobAdvertisementService = new JobAdvertisementService();
    jobAdvertisementService.deleteJobAdvertisement(jobAdvertisement).then(alert("Success"));
    window.location.reload();
}
  ...
  return (
    ...
    >
      ...
      <Modal.Actions>
        ...
        <Button color='green' inverted onClick={() => {setOpen(false);setActiveTrue(id)}}>
          ...
          <Table.Body>
            {filteredJobAdvertisements.map((jobAdvertisement) => (
              <Table.Row key={jobAdvertisement.id}>
                ...
                <Table.Cell>
                <Button icon="check" positive onClick={()=>{setOpen(true); setId(jobAdvertisement.id);}}></Button>
                </Table.Cell>
                <Table.Cell>
                <Popup trigger={<Button onClick={()=>deleteJobAdvertisement(jobAdvertisement)} negative icon='remove' />} content="Delete this job advertisement" basic/>
                </Table.Cell>
              </Table.Row>
            ))}
          </Table.Body>
